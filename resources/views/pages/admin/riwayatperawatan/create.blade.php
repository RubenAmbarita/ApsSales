...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Form Jadwal Perawatan</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.riwayatperawatan.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="treatment_date" id="treatment_date" class="form-control" value="{{ old('treatment_date') }}" required>
                </div>
                <div class="form-group">
                    <label for="treatment_type">Tipe Perawatan</label>
                    <input type="text" name="treatment_type" id="treatment_type" class="form-control" value="{{ old('treatment_type') }}" placeholder="Preventive / Corrective / Upgrade" required>
                </div>
                <div class="form-group">
                    <label for="description">Deskripsi</label>
                    <textarea name="description" id="description" rows="3" class="form-control" placeholder="Catatan detail perawatan">{{ old('description') }}</textarea>
                </div>
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label for="cost">Biaya</label>
                        <input type="text" name="cost" id="cost" class="form-control" value="{{ old('cost') }}" placeholder="Contoh: Rp 1.500.000" inputmode="numeric" autocomplete="off">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="long_warranty" id="long_warranty" class="form-control" value="{{ old('long_warranty') }}" placeholder="Contoh: 6 bulan">
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  input.addEventListener('input', function(){
    ...
    applyFormat();
    // Letakkan kursor di akhir agar tidak melompat aneh saat format
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(serverSelect){
    var ts = new TomSelect(serverSelect, {
      ...
      items: [], // pastikan tidak ada pilihan default saat halaman dibuka
